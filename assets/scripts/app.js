...
function printLogHandler () {
    //givs the amount of item in the array  of let i = 0; i < battleLog.length; i++ 
    ...
    // Do also get access to the index of an array
    // for (let i = 0; i < battleLog.length; i++) {
    //     // could use battleLog[0], but using i get all item in the array
    //     // that have index 0 up to its not more. You will get access to each
    //     //every on its own isted of get all the array in one array
    //     console.log(battleLog[i]);
    // }    
    // JS create new constant for every loop iteration not alter the previous one
    //but create a new one and we are not redeclaring the excisting one
    // it works only on array or string not numbers bolean and object
    //logEntry get access to the element not the index the code below
    //does not give information about the element index
    // to get information of the index create a variable outside of the for loop
    // let i = 0;
    ...
    // }
    // console.log(battleLog);
    // to get access to the keyvalue par use for in loop
    // let i = 0;
    // for (const logEntry of battleLog) {
    //     console.log (`#${i}`);
